window.onload = function(){
    ...
    async function getStates(){
        const states = await inviaRichiesta('GET', '/api/states');
        if(states){
            console.log(states);
            states.forEach(state => {
                $('<option>').val(state.name).text(`${state.name} [${state.stationcount} emittenti]`)
                .appendTo(lstRegioni);
    ...
    async function getRadios(){
        ...
        if(radios){
            ...
            radios.forEach((radio)=>{
                ...
                $('<img>').prop({src:radio.favicon, width: 40}).on('error', function(){
                ...
                $('<td>').appendTo(tr).append($('<img>').prop({src: './like.jpg', width:40}).on('click', function(){
                    addLike(radio.id);
                }));
            })
        }
